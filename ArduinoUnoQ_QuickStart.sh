...
stage_display_fix() {
  ...
  log "Writing $FIX_SCRIPT"
  sudo tee "$FIX_SCRIPT" > /dev/null <<FIX_DISPLAY_MODE_SH
#!/bin/sh
...
FIX_DISPLAY_MODE_SH
  ...
  sudo tee "$SERVICE_FILE" > /dev/null <<SYSTEMD_UNIT
[Unit]
...
stage_zsh() {
  ...
  sudo apt install -y zsh zsh-common zsh-autosuggestions zsh-syntax-highlighting
...
stage_motd() {
  ...
  if [[ -f "$motd_file" ]]; then
    sudo cp "$motd_file" "$motd_file.bak.$(date +%Y%m%d%H%M%S)"
    log "Backed up existing $motd_file"
  fi

  log "Writing $motd_file"
  sudo tee "$motd_file" > /dev/null <<'MOTD_CONTENT'
███╗   ██╗██╗██╗   ██╗██████╗  ██████╗ ██╗  ██╗██╗   ██╗
████╗  ██║██║██║   ██║██╔══██╗██╔═══██╗██║ ██╔╝██║   ██║
██╔██╗ ██║██║██║   ██║██████╔╝██║   ██║█████╔╝ ██║   ██║
██║╚██╗██║██║╚██╗ ██╔╝██╔══██╗██║   ██║██╔═██╗ ██║   ██║
